...
async function metPages(term,limit=10){
  try{
    const s=await fetch('https://collectionapi.metmuseum.org/public/collection/v1/search?hasImages=true&q='+encodeURIComponent(term));
    if(!s.ok)return[];
    const d=await s.json(),ids=(d.objectIDs||[]).slice(0,Math.max(limit*3,24));
    const objects=await Promise.all(ids.map(async id=>{try{const r=await fetch('https://collectionapi.metmuseum.org/public/collection/v1/objects/'+id);return r.ok?await r.json():null}catch{return null}}));
    return objects.filter(x=>x&&x.primaryImageSmall&&x.isPublicDomain).slice(0,limit).map((x,n)=>({
      pageid:'met-'+x.objectID,title:'File:'+(x.title||('The Met object '+x.objectID)),_source:'met',
      imageinfo:[{url:x.primaryImage||x.primaryImageSmall,thumburl:x.primaryImageSmall,descriptionurl:x.objectURL,
        extmetadata:{LicenseShortName:{value:'Public domain'},UsageTerms:{value:'Public domain'},Artist:{value:x.artistDisplayName||x.artistDisplayBio||'The Metropolitan Museum of Art'},Credit:{value:x.creditLine||'The Metropolitan Museum of Art'},ImageDescription:{value:[x.objectName,x.title,x.objectDate,x.medium].filter(Boolean).join(' · ')}}}]
    }));
  }catch{return[]}
}
...
async function search(){const term=q.value.trim();if(!term)return;const t=T[lang];results.innerHTML='';filters.hidden=true;licenseFilter.innerHTML='';status.textContent=t.searching;try{
  ...
  if($('#exactSearch').checked){
    ...
    pages=chosen.slice(0,24).map(x=>x.page);pages.forEach(p=>p._source='commons');const searchName=entity.english||term;const [loc,met,smithsonian,nhm]=await Promise.all([locPages(searchName,12,allowArt),metPages(searchName,12),smithsonianPages(searchName,18,allowArt),nhmPages(searchName,15)]),mixed=[];for(let i=0;i<50&&mixed.length<50;i++){if(pages[i])mixed.push(pages[i]);if(loc[i])mixed.push(loc[i]);if(met[i])mixed.push(met[i]);if(smithsonian[i])mixed.push(smithsonian[i]);if(nhm[i])mixed.push(nhm[i])}pages=mixed.slice(0,50);
  }else{
    const commons=await commonsPages(term+' filetype:bitmap',24);commons.forEach(p=>p._source='commons');const [loc,met,smithsonian,nhm]=await Promise.all([locPages(term,12),metPages(term,12),smithsonianPages(term,18,false),nhmPages(term,15)]);pages=[];for(let i=0;i<50&&pages.length<50;i++){if(commons[i])pages.push(commons[i]);if(loc[i])pages.push(loc[i]);if(met[i])pages.push(met[i]);if(smithsonian[i])pages.push(smithsonian[i]);if(nhm[i])pages.push(nhm[i])}
